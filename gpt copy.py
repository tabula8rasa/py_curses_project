...
def run(stdscr: curses.window) -> None:
    ...
    while True:
        ...
        for p in particles:
            ...
            # Пока жива голова — частица летит и оставляет хвост
            if p.age <= HEAD_LIFETIME:
                p.show_head = True
                ...
                # Хвост постепенно укорачивается
                if p.trail:
                    p.trail.pop()

            # Если головы нет и хвост уже исчез — частица мертва
            if not p.show_head and not p.trail:
                p.alive = False
                continue
